...
struct Poly : vector<LL> {       // coefficients in [0, P)
  ...
  static Poly Interpolate(const vector<LL> &x, const vector<LL> &y) {
    ...
    for (int i = _n - 1; i > 0; --i)
      down[i] =
          down[i * 2].Mul(up[i * 2 + 1]).iadd(down[i * 2 + 1].Mul(up[i * 2]));
    return down[1];
  }
  ...
  static LL LinearRecursion(const vector<LL> &a, const vector<LL> &coef,
