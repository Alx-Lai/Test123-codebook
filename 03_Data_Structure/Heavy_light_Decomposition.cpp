struct Heavy_light_Decomposition {  // 1-base
  int n, ulink[10005], deep[10005], mxson[10005], w[10005], pa[10005];
  ...
  void add_edge(int a, int b, int w) { G[a].pb(pii(b, et)), G[b].pb(pii(a, et)), edge[et++] = w; }
  ...
  void cut(int u, int link) {
    ...
    cut(mxson[u], link);
    for (auto i : G[u])
      if (i.X != pa[u] && i.X != mxson[u]) cut(i.X, i.X);
  }
