...
class kd_tree {
 ...
 private:
  ...
  struct __cmp {
    int sort_id;
    inline bool operator()(const node *x, const node *y) const { return operator()(x->pid, y->pid); }
  ...
  inline bool isbad(node *o) { return size(o->l) > alpha * o->s || size(o->r) > alpha * o->s; }
 ...
 public:
  ...
  inline void clear() { clear(root), root = 0, maxn = 1; }
  ...
  inline const vector<point> &range(const point &mi, const point &ma) {
    in_range.clear();
    range(root, 0, mi, ma);
