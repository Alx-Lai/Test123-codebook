...
struct Tri {
  ...
  int num_chd() const { return !!chd[0] + !!chd[1] + !!chd[2]; }
...
struct Trig {  // Triangulation
  ...
  void add_point(Tri *root, pll const &p) {
    ...
    for (int i = 0; i < 3; ++i) t[i] = new (tris++) Tri(root->p[i], root->p[(i + 1) % 3], p);
    for (int i = 0; i < 3; ++i) edge(Edge(t[i], 0), Edge(t[(i + 1) % 3], 1));
    for (int i = 0; i < 3; ++i) edge(Edge(t[i], 2), root->edge[(i + 2) % 3]);
...
void go(Tri *now) {  // store all tri into triang
  ...
  vst.insert(now);
  if (!now->has_chd()) return triang.push_back(now);
