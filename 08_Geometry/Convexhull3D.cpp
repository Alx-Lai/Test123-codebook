struct CH3D {
  ...
  double dblcmp(Point &p, face &f) { return dot(cross3(P[f.a], P[f.b], P[f.c]), p - P[f.a]); }
  int g[N][N], num, n;
  Point P[N];
  ...
  void solve() {
    ...
    if ([&]() {
          for (int i = 1; i < n; ++i)
            if (abs(P[0] - P[i]) > eps) return swap(P[1], P[i]), 0;
          return 1;
        }() ||
        [&]() {
          for (int i = 2; i < n; ++i)
            if (abs(cross3(P[i], P[0], P[1])) > eps) return swap(P[2], P[i]), 0;
          return 1;
        }() ||
        [&]() {
          for (int i = 3; i < n; ++i)
            if (fabs(dot(cross(P[0] - P[1], P[1] - P[2]), P[0] - P[i])) > eps) return swap(P[3], P[i]), 0;
          return 1;
        }())
      return;
    for (int i = 0; i < 4; ++i) {
  ...
  double get_area() {
    double res = 0.0;
    if (n == 3) return abs(cross3(P[0], P[1], P[2])) / 2.0;
    for (int i = 0; i < num; ++i) res += area(P[F[i].a], P[F[i].b], P[F[i].c]);
    return res / 2.0;
  }
  double get_volume() {
    double res = 0.0;
    for (int i = 0; i < num; ++i) res += volume(Point(0, 0, 0), P[F[i].a], P[F[i].b], P[F[i].c]);
    return fabs(res / 6.0);
  }
  ...
  int polygon() {
    ...
    for (int i = 0, flag = 1; i < num; ++i, res += flag, flag = 1)
      for (int j = 0; j < i && flag; ++j) flag &= !same(i, j);
    return res;
  }
  Point getcent() {
    ...
    for (int i = 0; i < num; ++i)
      if (F[i].ok == true) {
        ...
        if (t2 > 0)
          ans.x += (p1.x + p2.x + p3.x + temp.x) * t2, ans.y += (p1.y + p2.y + p3.y + temp.y) * t2, ans.z += (p1.z + p2.z + p3.z + temp.z) * t2,
              v += t2;
      }
